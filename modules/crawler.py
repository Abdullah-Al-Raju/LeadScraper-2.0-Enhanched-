        ...
        async with AsyncHTTPClient() as http:
            ...
            if contact_urls:
                ...
                for contact_data in contact_results:
                    if contact_data and not isinstance(
                            contact_data, Exception):
                        ...
                        crawl_data['structured_data'].extend(
                            contact_data.get('structured_data', []))
                        crawl_data['mailto_links'].extend(
                            contact_data.get('mailto_links', []))
                        crawl_data['tel_links'].extend(
                            contact_data.get('tel_links', []))
            ...
            logger.info(
                f"Successfully crawled {1 + len(crawl_data['contact_pages'])} pages from {url}")

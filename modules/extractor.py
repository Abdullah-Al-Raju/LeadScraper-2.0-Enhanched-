...
def extract_contacts(crawl_data, category=None):
    ...
    contacts = extract_from_structured_data(
        crawl_data.get('structured_data', []))
...
def extract_from_structured_data(structured_data_list):
        ...
        for item in items:
            ...
            if item_type in [
                ...
                if item.get('email'):
                    email = item['email']
                    if is_valid_email(
                            email) and email not in contacts['email_addresses']:
                        contacts['email_addresses'].append(email)

                # Address
                address = item.get('address')
                if address and isinstance(address, dict):
                    if not contacts['street_address'] and address.get(
                            'streetAddress'):
    ...
    if contacts['phone_numbers'] or contacts['email_addresses']:
        logger.info(
            f"Extracted from structured data: {
                len(
                    contacts['phone_numbers'])} phones, {
                len(
                    contacts['email_addresses'])} emails")
...
First, understand what this business is. Then, intelligently extract ALL contact information you can find."""
    ...
    try:
        ...
        logger.info(
            f"🤖 AI Understanding: {
                understanding.get('type')} - {
                understanding.get('description')} (Confidence: {
        ...
        formatted['phone_numbers'] = [
            format_phone(p) for p in contacts.get(
                'phone_numbers', []) if p]
        formatted['email_addresses'] = [
            e for e in contacts.get(
                'email_addresses',
                []) if is_valid_email(e)]
        ...
        logger.info(
            f"🤖 AI extracted: {len(formatted['phone_numbers'])} phones, {len(formatted['email_addresses'])} emails")
...
def extract_with_regex(text, mailto_links=None, tel_links=None):
    ...
    logger.info(
        f"Regex extraction: {len(contacts['phone_numbers'])} phones, {len(contacts['email_addresses'])} emails")
...
def _get_category_context(category):
    ...
    if any(
        word in category_lower for word in [
            'restaurant',
            'cafe',
            'coffee',
            'food',
...
---"""
    ...
    try:
        ...
        logger.info(
            f"Category-aware AI extraction successful: {
                len(
                    formatted['phone_numbers'])} phones, {
                len(
                    formatted['email_addresses'])} emails")

...
AI plans strategy, decides searches, executes via DuckDuckGo, adapts based
on results
"""
...
def discover_restaurants(location, quantity=100, category='restaurant'):
    ...
    """
    logger.info(
        f"AI AGENT STARTING: Find {quantity} {category}s in {location}")
    ...
    for i, query in enumerate(queries, 1):
        ...
        try:
            ...
            if names:
                ...
                logger.info(
                    f"  AI extracted {len(names)} names "
                    f"(total: {len(all_names)})"
                )

            # AI decides if we should continue
            if len(all_names) >= quantity * 1.5:
                logger.info(
                    f"AI DECISION: Enough data ({len(all_names)} names), "
                    "stopping"
                )
                break
...
- Be intelligent and strategic"""
    ...
    user_prompt = f"""Plan a search strategy to find {quantity} {category} \
names in {location}.

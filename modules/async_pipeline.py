...
async def _extract_search_results(search_term, city, category):
    try:
...
async def _extract_social_search(search_term, city, platform):
    try:
        ...
        logger.warning(f"{platform.capitalize()} extraction failed: {e}")
        return None


async def _run_extractions(website_url, search_term, city, category):
    extraction_tasks = []
    ...
    # Task 2: Search results extraction
    extraction_tasks.append(_extract_search_results(search_term, city, category))

    # Task 3: Facebook search
    extraction_tasks.append(_extract_social_search(search_term, city, 'facebook'))

    # Task 4: Instagram search
    extraction_tasks.append(_extract_social_search(search_term, city, 'instagram'))
...
def _update_completion_status(sheet, row_number, confidence, stats, progress):
    if confidence >= 80:
...
async def process_business_async(sheet, row, progress, stats, cache, semaphore):
    ...
    async with semaphore:  # Limit concurrency
        ...
        try:
            ...
            sources_data = await _run_extractions(website_url, search_term, city, category)
            ...
            _update_completion_status(sheet, row_number, confidence, stats, progress)

...
async def process_business_async(
        sheet,
        row,
        progress,
        stats,
    ...
    async with semaphore:  # Limit concurrency
        ...
        try:
            ...
            logger.info(
                f"Running {
                    len(extraction_tasks)} extractions in parallel...")
            ...
            if not sources_data:
                update_status(
                    sheet,
                    row_number,
                    config.STATUS_EXTRACTION_FAILED)
            ...
            display.print_completion_separator(
            ...
            logger.error(
                f"Error processing {display_name}: {e}",
                exc_info=True)
...
async def run_pipeline_async(sheet, rows, cache):
    ...
    logger.info(
        f"\nASYNC MODE: Processing {
            len(rows)} businesses with {MAX_CONCURRENT_BUSINESSES} concurrent!")

...
def merge_multi_source_data(sources_data):
    ...
    valid_sources = [s for s in sources_data if s and (
        s.get('business_name') or s.get('phone_numbers') or s.get('email_addresses'))]
    ...
    merged['business_name'] = _resolve_single_field(
        field_sources['business_name'])
    merged['street_address'] = _resolve_single_field(
        field_sources['street_address'])
    ...
    merged['facebook'] = _resolve_single_field(
        field_sources['facebook']) or merged['source_urls'].get('facebook')
    merged['instagram'] = _resolve_single_field(
        field_sources['instagram']) or merged['source_urls'].get('instagram')
    merged['business_hours'] = _resolve_single_field(
        field_sources['business_hours'])

    # Merge lists (prioritize most common)
    merged['phone_numbers'] = _resolve_list_field(
        field_sources['phone_numbers'])
    merged['email_addresses'] = _resolve_list_field(
        field_sources['email_addresses'])

    # Calculate confidence score
    merged['confidence_score'] = calculate_confidence_score(
    ...
    logger.info(
        f"Merged profile: {
            merged['business_name']} - Confidence: {
...
def _resolve_single_field(field_data):
    ...
    # Sort by priority
    sorted_data = sorted(field_data, key=lambda x: source_priority.get(
        x[1].replace('directory:', '').split(':')[0], 0), reverse=True)
...
def calculate_confidence_score(merged_data, field_sources):
    ...
    if phone_dict:
        max_phone_sources = max(len(sources)
                                for sources in phone_dict.values())
    ...
    if email_dict:
        max_email_sources = max(len(sources)
                                for sources in email_dict.values())

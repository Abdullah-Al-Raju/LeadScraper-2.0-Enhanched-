...
def get_unprocessed_rows(sheet):
    ...
    try:
        ...
        for idx, record in enumerate(
                all_records, start=2):  # Start at 2 (headers in row 1)
            status = record.get('Status', '').strip()

            # Process if status is empty or marked for retry
            if not status or status == config.STATUS_RETRY:
                # Support both old format (Business Name) and new format
                # (Category)
                business_name = record.get('Business Name', '').strip()
                category = record.get(
                    'Category', '').strip() or record.get(
                    'Business Type', '').strip()

                # Use category if provided, otherwise fall back to business
                # name
...
def write_to_output(sheet, contacts):
    ...
    try:
        ...
        row_data = [
            ...
            ', '.join(contacts.get('phone_numbers', [])
                      ) if contacts.get('phone_numbers') else '',
            ', '.join(contacts.get('email_addresses', [])
                      ) if contacts.get('email_addresses') else '',
            ...
            ', '.join(contacts.get('sources_used', [])) if contacts.get(
                'sources_used') else '',  # Sources
...
def load_existing_results(sheet):
    ...
    try:
        ...
        logger.info(
            f"Loaded {
                len(_processed_hashes)} existing results for duplicate detection")
...
def initialize_output_sheet(sheet):
    ...
    try:
        ...
        if not first_row or first_row[0] != 'Business Name':
            ...
            logger.info(
                "Initialized output sheet with enhanced headers (includes source URLs)")
    ...
    try:
        ...
        sorted_rows = sorted(
            ...
            key=lambda row: float(
                row[confidence_idx]) if row[confidence_idx] and str(
                row[confidence_idx]).replace(
                '.',
                '').isdigit() else 0,
            reverse=True)
        ...
        if sorted_rows:
            worksheet.append_rows(
                sorted_rows, value_input_option='USER_ENTERED')

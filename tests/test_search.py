...
class TestSearchHelpers(unittest.TestCase):
    ...
    def test_build_search_query(self):
        # Only business name
        self.assertEqual(_build_search_query("Bob's Burgers", include_contact=False), '"Bob\'s Burgers"')

        # Business name and city
        self.assertEqual(_build_search_query("Bob's Burgers", city="New York", include_contact=False), '"Bob\'s Burgers" "New York"')

        # With contact keywords
        self.assertEqual(_build_search_query("Bob's Burgers", include_contact=True), '"Bob\'s Burgers" contact OR phone OR email')

        # All together
        self.assertEqual(_build_search_query("Bob's Burgers", city="New York", include_contact=True), '"Bob\'s Burgers" "New York" contact OR phone OR email')

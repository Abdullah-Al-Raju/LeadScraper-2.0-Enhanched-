...
from modules.search import validate_search_setup
...
def run_pipeline():
    ...
    try:
        ...
        if cache.exists():
            display.info(
                "Found cached progress - checking if resume available...")
            ...
            if completed_rows:
                display.success(
                    f"Can resume from {
                        len(completed_rows)} completed items!")
                # Filter out already completed rows
                rows = [r for r in rows if r['row_number']
                        not in completed_rows]
        ...
        if use_async:
            ...
            cache.clear()
            display.success(
                "All businesses processed successfully - cache cleared!")
            ...
            logger.info(
                f"Total execution time: {minutes} minutes, {seconds} seconds")
        ...
        if not use_async and config.ENABLE_DISCOVERY_MODE and _is_discovery_mode(
                rows):
            ...
            logger.info(
                f"Starting discovery: {quantity} {category}s in {location}")
            ...
            logger.info(
                f"Discovered {
                    len(restaurants)} restaurants with contact info")
            ...
            rows = get_unprocessed_rows(sheet)
            logger.info(
                f"Proceeding to process {
                    len(rows)} discovered restaurants")
...
def process_business(sheet, row, progress, stats, cache=None):
    ...
    # What we'll actually search for
    ...
    try:
        # Check for duplicate (use actual business name if we have it,
        ...
        try:
            ...
            search_data = extract_from_search_results(
                search_term, city, category)
            if search_data:
        ...
        if source_urls.get('facebook'):
            try:
                ...
                fb_data = extract_from_social_search(
                    search_term, city, 'facebook')
                if fb_data:
        ...
        if source_urls.get('instagram'):
            try:
                from modules.sources.search_result_extractor import extract_from_social_search
                ig_data = extract_from_social_search(
                    search_term, city, 'instagram')
                if ig_data:
        ...
        if source_urls.get('website'):
            try:
                ...
                is_dir, dir_type = is_extractable_directory(
                    source_urls['website'])

                if is_dir:
                    # Extract FROM the directory
                    logger.info(f"Found {dir_type} directory - extracting...")
                    dir_data = extract_from_directory(
                        source_urls['website'], dir_type)
                    if dir_data:
                    ...
                    if crawl_data:
                        ...
                        website_contacts = extract_contacts(
                            crawl_data, category=category)
                        if website_contacts:
        ...
        # Set business name if not extracted (AI should extract the real name
        # from website)
        if not contacts.get('business_name'):
            # Use search_term as fallback, but AI should have found the real
            # name
        ...
        logger.info(
            f"  - Business: {contacts.get('business_name', 'Unknown')}")
        logger.info(f"  - Phones: {len(contacts.get('phone_numbers', []))}")
        logger.info(f"  - Emails: {len(contacts.get('email_addresses', []))}")
        logger.info(
            f"  - Address: {'Yes' if contacts.get('street_address') else 'No'}")
        logger.info(f"  - Confidence: {confidence}%")
        logger.info(
            f"  - Sources: {', '.join(contacts.get('sources_used', []))}")
        logger.info(
            f"  - Verified: {', '.join(contacts.get('verified_fields', []))}")
...
def _validate_config():
    ...
    if not config.SERVICE_ACCOUNT_FILE:
        logger.error(
            "SERVICE_ACCOUNT_FILE not set. Please configure in .env file")
        valid = False

    if not config.OPENROUTER_API_KEY:
        logger.warning(
            "OPENROUTER_API_KEY not set. AI extraction will be disabled.")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f'Number of businesses to find (default: {
            config.DEFAULT_DISCOVERY_QUANTITY})')
...
def run_cli_discovery(location, quantity, category):
    ...
    display.section_header(
        f"AI AGENT MODE: {quantity} {category}s in {location}")
    ...
    display.info(
        f"Populating input sheet with {
            len(restaurants)} businesses...")
    populate_input_sheet(workbook, restaurants)

    display.section_header(
        f"DISCOVERY COMPLETE: {
            len(restaurants)} businesses added")
...
if __name__ == "__main__":
    ...
    display.success(
        f"AI Manager initialized with {len(config.OPENROUTER_MODELS)} models")
